...
const отрисоватьШапку = () => {
  ...
  Ы.элемент(телоДокумента).создать.блок({содержимое: `Данная библиотека - эксперимент. Автор прекрасно понимает, что нарушает несколько паттернов разработки, например пищет код не на английском. Поэтому <b>крайне не рекомендуется использовать данную библиотеку в реальных проектах</b>
...
const отрисоватьДокументацию = (родитель) => {
  ...
  // ключиСправки.forEach((ключ) => {
  //   Ы.элемент('.менюДокументации').создать.ссылка({ содержимое: ключ, класс: 'пунктМенюДокументации', ссылка: `#документация/${ключ}`})
  // });
  ...
  полеПоиска.событие('отпусканиеКлавиши', (event) => {
    ...
    // console.log(полеПоиска.значение())
  });
