...
const отрисоватьОсновнойКонтент = () => {
  ...
  const первыйБлок = { 
    содержимое: `
    <div class="предупреждение">Данная библиотека - эксперимент. Автор прекрасно понимает, что нарушает несколько паттернов разработки, например пищет код не на английском. Поэтому <b>крайне не рекомендуется использовать данную библиотеку в реальных проектах</b></div>
